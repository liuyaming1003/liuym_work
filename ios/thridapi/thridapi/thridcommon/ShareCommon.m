...
#define WX_APPID     @"wxc1063474755a5f24"
#define WX_APPSECRET @"b889277edd351b2d64966218cf55d0b7"
#define WX_LOGON     @"wx_logon"

#define QQ_LOGON     @"qq_logon"
#define QQ_APPID     @"1104663467"
...
static ShareCommon *sg_instance = nil;

@interface ShareCommon()<TencentLoginDelegate, TencentSessionDelegate, WXApiDelegate>

@property (strong, nonatomic) TencentOAuth *qqOAuth;
@property (nonatomic) ShareCommonType commonType;
...
- (id)init
{
    self = [super init];
    if(self){
        self.commonType = (int)[[NSUserDefaults standardUserDefaults] integerForKey:LOGON_TYPE];
...
- (void)setCommonType:(ShareCommonType)type
{
    //保存类型
    _commonType = type;
...
{
    if(sg_instance == nil){
        sg_instance = [[ShareCommon alloc] init];
    }
    return sg_instance;
}
...
+ (NSDictionary *)getSupportShareApp
{
    ...
    [dict setObject:[NSNumber numberWithBool:[WXApi isWXAppInstalled]] forKey:WX_INSTALL];
    
    [dict setObject:[NSNumber numberWithBool:[TencentOAuth iphoneQQInstalled]] forKey:QQ_INSTALL];
...
//            }
        ...
        case ShareCommonType_WX:
        {
            ...
            if(dict){
                ...
                if([self isValidTime:refresh_token_time expiration:refresh_token_expires]){
                    isValid = YES;
                }
            }
        }
...
- (void)logon:(ShareCommonType)type viewController:(UIViewController *)viewController permission:(NSArray *)array callBack:(void (^)(BOOL, id, NSError *))callBack
{
...
- (void)logout:(void (^)(BOOL, id, NSError *))callBack
{
    self.callBack = callBack;
    switch (self.commonType) {
        case ShareCommonType_QQ:
...
- (void)getUserInfo:(void (^)(BOOL, id, NSError *))callBack
{
    self.callBack = callBack;
    switch (self.commonType) {
        case ShareCommonType_QQ:
