...
@property (nonatomic) int panWidth;
@property (nonatomic, strong) NSString *filePath;

@end

@implementation SignNameSDK

- (void)setSN:(NSString *)filePath panColor:(UIColor *)panColor panWidth:(int)panWidth
{
    self.filePath = filePath;
    self.panColor = panColor;
...
{
    if (self = [super initWithCoder:aDecoder]) [self initSN];
    return self;
}

- (id)initWithFrame:(CGRect)frame context:(EAGLContext *)ctx
{
    if (self = [super initWithFrame:frame context:ctx]) [self initSN];
    return self;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (length > 2) {
        glBindVertexArrayOES(vertexArray);
        NSLog(@"length = %u, array = %u", length, vertexArray);
...
- (void)updateStrokeColor {
    ...
    if (effect && self.panColor && [self.panColor getRed:&red green:&green blue:&blue alpha:&alpha]) {
        effect.constantColor = GLKVector4Make(red, green, blue, alpha);
    } else if (effect && self.panColor && [self.panColor getWhite:&white alpha:&alpha]) {
